 ...
 * summary's nameColor ("&" codes from the API: the tier's NameColor, else the Default group's;
 * a team takes only the 16 named colors, so a hex color maps to the nearest one); everyone
...
public record TabListTeam(String name, NamedTextColor color) {
    ...
    public static TabListTeam resolve(boolean isOwner, boolean isStaff, UserSummary user) {
        ...
        NamedTextColor color = LegacyStyles.nameColor(user.nameColor(), NamedTextColor.GRAY);
