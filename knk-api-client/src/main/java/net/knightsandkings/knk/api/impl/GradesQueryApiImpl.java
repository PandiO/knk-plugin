...
import net.knightsandkings.knk.api.mapper.GradeMapper;
...
public class GradesQueryApiImpl implements GradesQueryApi {
    private static final Logger LOGGER = Logger.getLogger(GradesQueryApiImpl.class.getName());
    ...
    private static final String BASE_ENDPOINT = "/Grades";
    private static final String SEARCH_ENDPOINT = "/Grades/search";
    private static final int MAX_RESPONSE_SNIPPET_LENGTH = 200;

    public GradesQueryApiImpl(
            String baseUrl,
    ...
    @Override
    public CompletableFuture<Page<KnkGrade>> search(net.knightsandkings.knk.core.domain.common.PagedQuery query) {
            ...
            try {
                ...
                try (Response response = httpClient.newCall(request).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        throw new ApiException(url, response.code(), "Search grades failed", snippet);
                    }
                    ...
                    try {
                        PagedResultDto<GradeDto> result = objectMapper.readValue(
                                responseBody,
                                new TypeReference<PagedResultDto<GradeDto>>() {}
                        );
                        return GradeMapper.mapPagedList(result);
    ...
    @Override
    public CompletableFuture<KnkGrade> getById(int id) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                if (!response.isSuccessful()) {
                    String snippet = responseBody.substring(0, Math.min(responseBody.length(), MAX_RESPONSE_SNIPPET_LENGTH));
                    if (responseBody.length() > MAX_RESPONSE_SNIPPET_LENGTH) snippet += "...";
                    throw new ApiException(url, response.code(), "Get grade by ID failed", snippet);
                }
                ...
                try {
                    GradeDto dto = objectMapper.readValue(responseBody, GradeDto.class);
                ...
                ApiException apiEx = new ApiException(
                        url,
                        0,
                        "IO error during get grade by ID",
                        e.getClass().getSimpleName() + ": " + e.getMessage()
                );
                apiEx.initCause(e);
                throw apiEx;
