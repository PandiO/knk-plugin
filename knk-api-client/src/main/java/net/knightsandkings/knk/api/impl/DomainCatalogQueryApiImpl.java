...
import net.knightsandkings.knk.api.mapper.DomainCatalogMapper;
...
public class DomainCatalogQueryApiImpl implements DomainCatalogQueryApi {
    private static final Logger LOGGER = Logger.getLogger(DomainCatalogQueryApiImpl.class.getName());
    ...
    private static final String BASE_ENDPOINT = "/Domains";
    private static final String SEARCH_ENDPOINT = "/Domains/search";
    private static final int MAX_RESPONSE_SNIPPET_LENGTH = 200;

    public DomainCatalogQueryApiImpl(
            String baseUrl,
    ...
    @Override
    public CompletableFuture<Page<KnkDomainSummary>> search(net.knightsandkings.knk.core.domain.common.PagedQuery query) {
            ...
            try {
                ...
                try (Response response = httpClient.newCall(request).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        throw new ApiException(url, response.code(), "Search domains failed", snippet);
                    }
                    ...
                    try {
                        PagedResultDto<DomainSummaryDto> result = objectMapper.readValue(
                                responseBody,
                                new TypeReference<PagedResultDto<DomainSummaryDto>>() {}
                        );
                        return DomainCatalogMapper.mapPagedList(result);
                ...
                ApiException apiEx = new ApiException(
                        url,
                        0,
                        "IO error during search domains",
    ...
    @Override
    public CompletableFuture<KnkDomainSummary> getById(int id) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                if (!response.isSuccessful()) {
                    String snippet = responseBody.substring(0, Math.min(responseBody.length(), MAX_RESPONSE_SNIPPET_LENGTH));
                    if (responseBody.length() > MAX_RESPONSE_SNIPPET_LENGTH) snippet += "...";
                    throw new ApiException(url, response.code(), "Get domain by ID failed", snippet);
                }
                ...
                try {
                    DomainSummaryDto dto = objectMapper.readValue(responseBody, DomainSummaryDto.class);
                    return DomainCatalogMapper.toCore(dto);
                } catch (Exception parseEx) {
                    LOGGER.warning("Failed to parse response: " + responseBody);
                    throw new ApiException(
                            url,
                ...
                ApiException apiEx = new ApiException(
                        url,
                        0,
                        "IO error during get domain by ID",
                        e.getClass().getSimpleName() + ": " + e.getMessage()
                );
                apiEx.initCause(e);
                throw apiEx;
